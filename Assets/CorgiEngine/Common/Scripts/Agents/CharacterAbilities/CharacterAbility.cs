...
{	
	...
	public class CharacterAbility : MonoBehaviour 
	{
		...
		/// 如果为true，则该能力可以像往常一样执行，否则将被忽略。例如，你可以使用它来解锁技能
		[Tooltip("如果为true，则该能力可以像往常一样执行，否则将被忽略。例如，你可以使用它来解锁技能")]
		...
		[Tooltip("包含所有阻塞移动状态的数组。如果角色处于这些状态之一并试图触发此能力，则不允许。例如，用于防止在空闲或游泳时使用此功能。")]
		public CharacterStates.MovementStates[] BlockingMovementStates;
		/// an array containing all the blocking condition states. If the Character is in one of these states and tries to trigger this ability, it won't be permitted. Useful to prevent this ability from being used while dead, for example.
		[Tooltip("包含所有阻塞条件状态的数组。如果角色处于这些状态之一并试图触发此技能，则不允许使用。例如，用于防止此技能在死亡时被使用。")]
