...
namespace MoreMountains.CorgiEngine
{
	...
	public class CharacterHandleWeapon : CharacterAbility 
	{
		...
		public override string HelpBoxText() { return "这个组件将允许你的角色拾取和使用武器。武器的作用在武器类中定义。" +
                "这只是描述握着武器的“手”的行为，而不是武器本身。在这里你可以为你的角色设置一个初始武器，允许武器拾取，并指定一个武器附件(你的角色内部的转换，" +
                "可以只是一个空的子游戏对象，或你的模型的子部分。"; }
		...
		[Tooltip("AmmoDisplay的ID应该更新，如果这是真的，这个动画将自动绑定到武器")]
		...
		[Tooltip("如果这是真的，你将不需要释放你的发射按钮来自动重新加载")]
        ...
        [Tooltip("如果设置为true，角色将被迫面对当前的武器方向")]
        ...
        [Tooltip("如果这是正确的，水平瞄准将在射击时倒立，同时紧贴墙壁，以射击远离墙壁")] 
