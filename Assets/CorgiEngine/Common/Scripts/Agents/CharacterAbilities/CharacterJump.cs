...
namespace MoreMountains.CorgiEngine
{
	...
	{	
		...
		public override string HelpBoxText() { return "该组件处理跳转。在这里你可以定义跳跃高度，跳跃是否与按压长度成正比，最小空中时间(如果玩家松开跳跃按钮，角色在空中停留的时间)，" +
                "跳跃窗口持续时间(从悬崖上掉下来后仍有可能跳跃的时间)，跳跃限制，角色在不接触地面的情况下能够执行的跳跃次数，当退出单向平台或移动平台时，多长时间应该禁止碰撞。"; }
		...
		[Tooltip("如果这是真的，这个角色就可以通过向下+跳跃的方式从一个平台上跳下来")]
		...
		[Tooltip("如果为真，跳跃持续时间/高度将与按钮按下的持续时间成比例")]
		...
		[Tooltip("跳跃时允许在空中停留的最短时间-用于压力控制跳跃")]
		...
		[Tooltip("当跳跃按钮被释放时，我们将修改当前速度的数量，跳跃时允许在空中的最小时间-这用于压力控制的跳跃")]
		...
		protected override void HandleInput()
		{
			...
			if ((InputBufferDuration > 0f) && (_controller.State.JustGotGrounded))
			{
				if ((_inputManager.JumpButton.TimeSinceLastButtonDown < InputBufferDuration) && (Time.time - _lastJumpAt > InputBufferDuration)) 
				{
					...
					_jumpButtonPressed = (_inputManager.JumpButton.State.CurrentState ==
					                      MMInput.ButtonStates.ButtonPressed);
					_jumpButtonPressTime = Time.time;
					_jumpButtonReleased = (_inputManager.JumpButton.State.CurrentState !=
					                       MMInput.ButtonStates.ButtonPressed);
