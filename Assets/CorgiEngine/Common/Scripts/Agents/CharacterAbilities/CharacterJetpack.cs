...
{	
	...
	public class CharacterJetpack : CharacterAbility 
	{
		...
		public override string HelpBoxText() { return "将这个组件添加到角色中，它将能够激活喷气背包并通过关卡飞行。在这里你可以定义当喷气背包时所应用的力量，所使用的粒子系统，各种燃料信息，" +
                "以及当喷气背包充满燃料时所播放的声音"; }

		[Header("Jetpack")]

		/// the duration (in seconds) during which we'll disable the collider when taking off jetpacking from a moving platform
		[Tooltip("持续时间(以秒为单位)，在此期间，我们将禁用对撞机时，从一个移动的平台上起飞")]
