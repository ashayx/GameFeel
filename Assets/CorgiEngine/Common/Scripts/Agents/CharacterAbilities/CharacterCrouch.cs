...
{	
	...
	public class CharacterCrouch : CharacterAbility 
	{
		...
		public override string HelpBoxText() { return "该组件处理蹲伏和爬行行为。在这里，您可以确定蹲伏速度，以及碰撞器在蹲伏时是否应调整大小（例如爬行到隧道中）。如果需要，请在此处设置新尺寸。"; }

		[Header("Crawl")]

		/// if this is set to false, the character won't be able to crawl, just to crouch
		[Tooltip("如果设置为false，角色将无法爬行，只能蹲下")]
		...
		[Tooltip("蹲下时应用于碰撞器的大小（如果ResizeColliderWhenCrought为true，否则将忽略此值）请注意，蹲下时更改碰撞器的宽度可能会导致在斜坡上开始蹲下时出现故障，最好不要更改它")]
		...
		[Tooltip("如果该值为false，则角色必须停止移动才能开始蹲伏")]
		...
		[MMReadOnly]
		[Tooltip("如果这是真的，那么角色是蹲着的，头上有一个障碍物，阻止它再次站起来")]
		...
		[Tooltip("是否移动摄像机目标，该目标将用作Cinemachine虚拟摄像机的焦点")]
