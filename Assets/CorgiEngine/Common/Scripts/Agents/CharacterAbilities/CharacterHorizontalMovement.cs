...
{	
	...
	public class CharacterHorizontalMovement : CharacterAbility 
	{
		...
		public override string HelpBoxText() { return "该组件处理基本的左/右移动、摩擦和地面撞击检测。在这里，您可以定义标准移动速度、行走速度以及角色在跳跃/摔倒后落地时使用的效果。"; }
		...
		[Tooltip("如果这是真的，将从输入源获取输入，否则必须通过 SetHorizontalMove()")]
		...
		[Tooltip("如果这是真的，则不会对运动施加加速度，这将立即达到全速（想想Megaman运动）")]
		...
		[Tooltip("考虑输入后的阈值（通常为0.1f，以消除小操纵手柄噪声）")]
		public float InputThreshold = 0.1f;
        /// how much air control the player has
        [Range(0f, 1f)]
		[Tooltip("player有多少空中控制")]
		...
		[Tooltip("在接触地面时播放反馈之前，角色必须悬空的持续时间（秒")]
		...
		[Tooltip("与墙壁横向碰撞时，是否应将状态重置为空闲状态")]
