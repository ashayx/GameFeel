...
{	
	...
	{		
		...
		public override string HelpBoxText() { return "该组件允许角色冲刺。在这里，您可以定义冲刺应该覆盖的距离，在冲刺过程中施加多少力（影响其持续时间），是否应在冲刺退出时重置力（否则将施加惯性）。" +
                "然后，您可以定义如何选择冲刺的方向，是否应自动翻转角色以匹配冲刺的方向，以及如果输入稍有错误，是否要更正轨迹以防止固定角色不冲刺。" +
                "最后，你可以调整短跑结束和下一个短跑开始之间的冷却时间。"; }
        ...
        [Tooltip("如果这是真的，力将在仪表板退出时重置（消除惯性）")]
        ...
        [Tooltip("如果这是真的，则角色在冲刺时将翻转，并朝向冲刺的相反方向")]
        ...
        [Tooltip("如果这是真的，将防止角色在已经接地时冲入地面")]
        ...
        [Tooltip("仅当冲刺不是无限长时，角色可以执行的连续冲刺数量")]
        ...
        [Tooltip("在时间重置模式下，仅当冲刺不是无限长时，剩余冲刺量重置后的持续时间（秒）")]
