...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		[Header("Collisions")]
		[MMInformation("您需要定义此角色将考虑可行走平台/移动平台等的层。默认情况下，您希望平台、移动平台、单向平台、移动新平台按此顺序排列。", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
        ...
        [Header("Safe Mode")]
        [MMInformation("如果将SafeSetTransform设置为true，则直接修改角色位置的功能（如CharacterGrip）将执行检查，以确保有足够的空间执行此操作。" +
            "这在默认情况下不会启用，因为这会带来（小）性能成本，通常可以通过安全级别的设计来避免。", MoreMountains.Tools.MMInformationAttribute.InformationType.Info, false)]
		...
		[Tooltip("设置变换位置时是否执行其他检查。性能稍贵，但也更安全。 ")]
        ...
        [Tooltip("如果这是真的，这个控制器将在init上自动设置一些物理设置，以确保它们是正确的")]
		...
		[Header("Raycasting")]
		[MMInformation("在这里，您可以定义水平和垂直投射的光线数量。您希望它们彼此尽可能远，但距离足够近，使任何障碍物或敌人都无法容纳在两条光线之间",MoreMountains.Tools.MMInformationAttribute.InformationType.Info, false)]
		...
		[Tooltip("所有光线投射都增加了一个小值，以适应边缘情况")]
		...
		[Tooltip("默认情况下，用于恢复正常大小的光线投射长度将根据角色的正常/站立高度自动生成，但在这里可以指定不同的值")]
		...
		[Tooltip("如果这是真的，将执行额外的boxcast以防止通过平台")]
		public bool PerformSafetyBoxcast = false;

        [Header("Stickiness")]
		[MMInformation("在这里，您可以定义在斜坡上行走时是否希望角色保持在斜坡上，以及光线投射处理的长度（0表示自动长度", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]

        /// If this is set to true, the character will stick to slopes when walking down them
		[Tooltip("如果该选项设置为true，则角色在斜坡上行走时将粘附在斜坡上")]
		public bool StickToSlopes = false;
		/// The length of the raycasts used to stick to downward slopes
		[Tooltip("用于粘附到向下坡度的光线投射的长度")]
		...
		[Header("Safety")]
		[MMInformation("在这里，你可以授权你的控制器开始旋转。这将改变其重力方向。更安全的做法是打开此安全设置，改用CharacterGravity技能。", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]

        /// if this is true, gravity ability settings will be automatically set. It's recommended to set that to true.
		[Tooltip("如果这是真的，重力能力设置将自动设置。建议将其设置为true。")]
